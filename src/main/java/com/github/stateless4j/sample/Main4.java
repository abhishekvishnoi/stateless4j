...
public class Main4 {
    ...
    // A mock of a cache ..
    static HashMap<String, StateMachine<String, String>> cache = new HashMap<>();
    ...
    private static void fireInitialTransition(StateMachineConfig<String, String> config){
        ...
        var xyz =  stateMachine.configuration();

        var xyz1 = stateMachine.getState();
        ...
        // generate UUID
        cache.put("random-config-uuid" , stateMachine);
    }

    private static void fireNextransition(String transistion){
         ...
         */
        var stateMachine = cache.get("random-config-uuid");

        var xyz =  stateMachine.configuration();

        stateMachine.configure("RUNNING");
